...
const Page = () => {
    ...
    // would change to the getOptions below once we build form components for this project
     ...
     React.useEffect(() => {
        const cntryCd = formData.country || formData.destination.country;
        if(cntryCd){
            const variables = {
                cntryCd,
                max: "1000"
    ...
    const getOptions = ({ searchInput = '' }) => {
        ...
        console.log('inside international', { searchInput });
    ...
    const getRate = React.useCallback((e) => {
        ...
        const variables = {
            origin: {
                ...
                city: formData.origin.state

            }, destination: {
                country: formData.country || formData.destination.country,
                state: formData.city,
                city: formData.city
            },
            items,
                ...
                <form onSubmit={getRate}>
                    ...
                    <div style={{margin: "1rem"}}>
                        ...
                        <select id="city" name={"city"} onChange={handleOnchange}>
                            <option>select destinaton city</option>
