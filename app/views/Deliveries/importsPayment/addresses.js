...
const Page = ({onItemSelected, dependencies}) => {
    ...
    const filterAddresses = _.filter(dependencies.addresses, (address) => ["NG"].includes(address.country.code))

    return (
        <div className={"max-w-3xl px-2 mx-auto"}>
            {filterAddresses.map((opt, idx) => {
                ...
                return(
                    <div 
                        ...
                        <div className={"col-span-3"}>
                            ...
                            <p className={"text-xs text-textGrey"}>{subText}</p>
                        </div>
