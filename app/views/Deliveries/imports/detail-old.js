...
const Page = ({query}) => {
    ...
    return (
        <TabLayout pageTitle={"Import Detail"}>
            <div className={"flex flex-col max-w-lg px-4 mx-auto"}>
                ...
                <div className={"border-b pb-4 border-textGrey border-opacity-10"}>
                    <p className={"text-primary uppercase text-xs mb-1 tracking-wider"}>{`${formatString('FROM', 'uppercase')}`}</p>
                    <p className={"tracking-wide text-left text-xl font-semibold"}>{`${source}`}</p>
                    <p className={"text-xs text-textGrey tracking-wider"}>{`${formatString( origin?.country?.name || '', 'uppercase', )}`}</p>
                </div>
                <div className={"border-b py-4 border-textGrey border-opacity-10"}>
                    <p className={"text-xs text-left tracking-wider text-textGrey"}>{`${formatString( description || 'Content description not provided', 'sentence', )}`}</p>
                </div>
                <div className={"border-b py-4 border-textGrey border-opacity-10"}>
                    <p className={"text-textGrey uppercase text-xs mb-1 tracking-wider"}>{'DELIVERING TO'}</p>
                ...
                <div className={"flex justify-between border-b py-4 border-textGrey border-opacity-10"}>
                    <p className={"text-xs text-textGrey tracking-wider"}>{'TRACKING CODE'}</p>
                    <p className={"text-xs font-medium tracking-wider"}>{`${formatString( code || '-', 'uppercase',)}`}</p>
                </div>
                <div className={"flex justify-between border-b py-4 border-textGrey border-opacity-10"}>
                    <div className={"flex flex-col"}>
                        <p className={"text-xs text-textGrey tracking-wider mb-2"}>{'STATUS'}</p>
                        <p className={"text-xs tracking-wider overflow-ellipsis text-left"}>{`${formatString(status?.name, 'uppercase')}`}</p>
                    </div>
                    <div>
                        <Button
                ...
                <div className={"flex justify-between border-b py-4 border-textGrey border-opacity-10"}>
                    <p className={"text-xs text-right text-textGrey tracking-wider"}>{'DECLARED WEIGHT (KG)'}</p>
                    <p className={"text-xs text-left font-semibold tracking-wider"}>{`${formatNumber(weight, '0.0')}`}</p>
                </div>
                <div className={"flex justify-between border-b py-4 border-textGrey border-opacity-10"}>
                    <p className={"text-xs text-right text-textGrey tracking-wider"}>{`DECLARED VALUE (${currency})` || `DECLARED VALUE (NGN)`}</p>
                    <p className={"text-xs text-left font-semibold tracking-wider"}>{`${formatNumber(total_value, '0,0.00')}` || '-'}</p>
                </div>
                <div className={"flex justify-between border-b py-4 border-textGrey border-opacity-10"}>
                    <p className={"text-xs text-right text-textGrey tracking-wider"}>{'IMPORT FEE'}</p>
                    <p className={"text-xs text-left font-semibold tracking-wider"}>{`${formatNumber(import_fee, '0,0.00')}`}</p>
                </div>
                <div className={"flex justify-between border-b py-4 border-textGrey border-opacity-10"}>
                    <p className={"text-xs text-right text-textGrey tracking-wider"}>{'AMOUNT PAID'}</p>
