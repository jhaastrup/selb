...
const ItemComponent = ({item, onItemSelected}) => {
    ...
    return (
        <div 
            ...
            <div>
                <p className={"text-xs text-left tracking-wider mb-1"}>{`${formatString(bank_code ?? '', 'uppercase')}`}</p>
...
const Page = () => {
    ...
    const onItemSelected = (item) => {
        ...
        const { pk: id } = item;
        router.push({
            pathname: "/account/activities/topups/[id]",
            query: {id}
        })
    };
    ...
    if (results.length === 0) {
        return (
            <TabLayout pageTitle={"Topups"} pathname={router.pathname}>
                <div className={"flex flex-col max-w-lg mx-auto h-screen justify-center items-center"}>
                    <div>
                        <p className={"text-center"}>{'No topup history'}</p>
                        <p className={"text-center text-textGrey"}>{'Wallet topups will appear here.'}</p>
                    </div>
                    <Spacer className={"block h-5"} />
                    <div>
                        <Button
