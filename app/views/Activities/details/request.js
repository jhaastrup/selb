...
import { REQUEST_DETAIL } from '../modules/queries';
...
const Page = ({query}) => {
    ...
    const { loading, data, error, refetch } = useQuery(REQUEST_DETAIL, {
    ...
    const {sender, target, status, code, amount, date_created, description} = data.request
    const { name } = data.me;
    ...
    return (
        <TabLayout pageTitle={"Request Detail"}>
            <div className={"flex flex-col max-w-lg px-4 mx-auto"}>
                ...
                <div className={"flex flex-row justify-start items-start py-4 border-b border-textGrey border-opacity-20"}>
                    <div className={"flex justify-center items-center uppercase text-xl font-semibold bg-backgroundGrey p-2 rounded-full "}>
                        {icontype}
                    </div>
