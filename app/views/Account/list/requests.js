...
import { Request } from "../detail"

const ItemComponent = ({ item, isSelected, onItemSelected, userObject }) => {
                    ...
                    <div className="flex items-center px-4 py-4 sm:px-6">
                        <div className="min-w-0 flex-1 flex items-center">
                            <div className="min-w-0 flex-1 px-4 md:grid md:grid-cols-2 md:gap-4">
                                <div>
                                    <p className={"text-xs font-bold text-left tracking-wider text-black mb-1"}>{`${formatString(currentProfile.name ?? '', 'uppercase')}`}</p>
    ...
    if (results.length === 0) {
        return (
            <div className={"flex flex-col max-w-lg mx-auto h-screen mt-32 items-center space-y-3"}>
                <div>
                    <p className={"text-center font-bold"}>{'No Request History'}</p>
                    <p className={"text-center text-gray-400"}>{'Payment requests will appear here.'}</p>
                </div>
                {/* <Spacer className={"block h-5"} /> */}
                <div>
                    <Button
