...
const Page = ({ dependencies }) => { 
  ...
  /*  useEffect(() => {
    ...
    const [usernameUpdate, { loading, data, error }] = useMutation(UPDATE_USERNAME, {
    onCompleted: (data) => {
  ...
  return (
   <div className='flex justify-between items-center'>
     <div>
       <h6 className="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2">{"Username"}</h6>
    ...
    onClick={()=>setPluginModalVisible(true)}
     ...
     > 
     <EditUsername dependencies={dependencies}/>
     </SimpleModal>
   </div>  
   );
};
